...
| `PROXY_SEGREDO` | **obrigatória** na Lambda; se faltar no `.env.aws`, o `deploy-aws.ps1` gera e grava uma. Opcional, mas recomendado: o **mesmo** valor à variável `PROXY_SEGREDO` do projeto Cloudflare Pages (Settings → Variables and Secrets, tipo *Secret*) e republicar o `infra/pages`. Com ele o limite de tentativas do login usa o IP real do visitante; sem ele, todos os visitantes pelo Pages parecem o mesmo IP (um pode travar o login do outro por 15 min) |
| `SESSAO_HORAS` | `24` |
| `SOMPO_API_KEY` | **VAZIA** |
